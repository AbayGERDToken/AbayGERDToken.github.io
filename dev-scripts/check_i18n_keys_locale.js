...
const lang = JSON.parse(fs.readFileSync(localePath,'utf8'));
...
for(const file of files){
  ...
  for(const k of keys){
    if (!hasKey(lang, k)) report.push({file: path.relative(root, file), key: k});
  }
}

if(report.length === 0){
  console.log(`No missing keys found in locales/${locale}.json for LocalizedText and t() usages.`);
  process.exit(0);
}

console.log(`Missing keys in locales/${locale}.json:`);
